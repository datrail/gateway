...
| `enforce` + `fallback: block` → the bound endpoint is forwarded, an unbound one is refused, and **no denial is reported** | The other half of `enforcement`, and the only place a caller's answer turns on something other than a policy. Both calls carry the same good ticket that is *allowed* through `gateway-enforce`, whose bundle differs only in its fallback — so the pair separates the fallback from the chain rather than from the ticket. Nothing is reported because a report names the policy that matched and none did; `policy_id` has no absent form. |
